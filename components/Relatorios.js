...
const Relatorios = () => {
  const translateOrderStatus = (status) => {
    ...
    return translations[status] || status; // Retorna o status original se não houver tradução
  };
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      const mapStatusToBackend = (status) => {
        ...
        return statusMapping[status] || status; // Retorna o valor original caso não encontre mapeamento
      };      
  
      switch (reportType) {
        case 'Pedidos':
          if (status) params.status = mapStatusToBackend(status);
  ...
  const generatePDF = () => {
    ...
    const getClientNameById = (id) => {
      ...
      return client ? client.name : 'todos';
    };
  
    const clientName = clientId ? getClientNameById(clientId) : 'todos';
    ...
    if (reportType === 'Pedidos') {
      ...
      doc.text(`Cliente: ${clientName ? clientName : 'todos'}`, 10, 51);
    ...
    if (reportType === 'Pedidos') {
      tableColumn = ['Cliente', 'Data do Pedido', 'Status', 'Valor Total (R$)'];
      ...
      tableColumn = ['Cliente', 'Total de Pedidos', 'Valor Total (R$)', 'Ticket Médio (R$)'];
      reportData.forEach((billing) => {
        const billingData = [
          billing.clientName,
          { content: (billing.totalOrders), styles: { halign: 'right' } },
      ...
      tableColumn = ['Produto', 'Categoria', 'Quantidade Vendida', 'Valor Total (R$)', 'Valor Médio (R$)'];
    ...
    doc.autoTable({
      ...
      didDrawPage: (data) => {
        ...
        // Adiciona o texto "X de Y" no canto superior direito
        doc.text(`${pageCount} de ${totalPagesExp}`, pageWidth - -30, 10, { align: 'right' });
      },
    });

    // Usa a substituição de marcador de página
                ...
                <tr>
                  {reportType === 'Pedidos' ? (
                    ...
                    <>
                      ...
                      <th style={{ textAlign: 'right' }}>Valor Total dos Pedidos (R$)</th>
                      <th style={{ textAlign: 'right' }}>Ticket Médio (R$)</th>
                    </>
                  ) : reportType === 'Produtos' ? (
                    <>
                      ...
                      <th style={{ textAlign: 'right' }}>Valor Total (R$)</th>
                      <th style={{ textAlign: 'right' }}>Valor Médio (R$)</th>
                    </>
                  ) : (
                    <>
                      <th>Nome</th>
                      <th>Descrição</th>
                      <th>Tipo</th>
                      <th style={{ textAlign: 'right' }}>Espessura (mm)</th>
                      <th style={{ textAlign: 'right' }}>Largura (m)</th>
                      <th style={{ textAlign: 'right' }}>Comprimento (m)</th>
                ...
                {reportType === 'Pedidos' &&
                  reportData.map((order, index) => (
                    <tr key={index}>
                      ...
                      <td style={{ textAlign: 'right' }}>{order.totalValue?.toFixed(2)}</td>
                      <td>
                        <ul>
                ...
                {reportType === 'Faturamento' &&
                  reportData.map((billing, index) => (
                    <tr key={index}>
                      ...
                      <td style={{ textAlign: 'right' }}>{billing.totalOrders}</td>
                ...
                {reportType === 'TodosProdutos' &&
                  reportData.map((product, index) => (
                    <tr key={index}>
                      ...
                      <td>{product.productType}</td>
                      <td style={{ textAlign: 'right' }}>{product.thickness?.toFixed(2)}</td>
                      <td style={{ textAlign: 'right' }}>{product.width?.toFixed(2)}</td>
                      <td style={{ textAlign: 'right' }}>{product.length?.toFixed(2)}</td>
                      <td style={{ textAlign: 'right' }}>{product.value?.toFixed(2)}</td>
                    </tr>
                  ))}
              </tbody>
            </table>
